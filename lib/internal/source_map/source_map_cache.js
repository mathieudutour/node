...
const kMaxMagicCommentValueLength = 16 * 1024 * 1024;
const kSourceMappingURL = 'sourceMappingURL';
...
function isMagicCommentWhiteSpace(code) {
  return code === CHAR_SPACE ||
         code === CHAR_TAB ||
         code === CHAR_FORM_FEED ||
...
function isMagicCommentLineTerminator(code) {
  return code === CHAR_LINE_FEED ||
         code === CHAR_CARRIAGE_RETURN ||
         code === 0x2028 ||
         code === 0x2029;
}

function isWorkerWithResourceLimits() {
  const {
    isMainThread,
    resourceLimits,
  ...
  return !isMainThread &&
         (resourceLimits[kMaxOldGenerationSizeMb] >= 0 ||
          resourceLimits[kMaxYoungGenerationSizeMb] >= 0);
}

/**
 * V8 stores sourceMappingURL values on Script objects after parsing. If a
 * constrained worker receives a huge inline source map, rejecting it before
 * compilation lets the worker fail without taking down the whole process.
 * @param {string} content - source content
 */
function maybeThrowOnLargeSourceMapURL(content) {
  if (content.length <= kMaxMagicCommentValueLength ||
      !isWorkerWithResourceLimits()) {
    return;
  }

  let position = 0;
  while ((position = StringPrototypeIndexOf(content, '//', position)) !== -1) {
    position += 2;

    const marker = StringPrototypeCharCodeAt(content, position);
    if (marker !== CHAR_HASH && marker !== CHAR_AT) {
      continue;
    }
    position++;

    if (!isMagicCommentWhiteSpace(StringPrototypeCharCodeAt(content, position))) {
      continue;
    }
    position++;

    if (!StringPrototypeStartsWith(content, kSourceMappingURL, position)) {
      continue;
    }
    position += kSourceMappingURL.length;

    if (StringPrototypeCharCodeAt(content, position) !== CHAR_EQUAL) {
      continue;
    }
    position++;

    while (isMagicCommentWhiteSpace(StringPrototypeCharCodeAt(content, position))) {
      position++;
    }

    const valueStart = position;
    while (position < content.length) {
      const code = StringPrototypeCharCodeAt(content, position);
      if (isMagicCommentWhiteSpace(code) || isMagicCommentLineTerminator(code)) {
        break;
      }
      if (position - valueStart >= kMaxMagicCommentValueLength) {
        throw new ERR_WORKER_OUT_OF_MEMORY('JS heap out of memory');
      }
      position++;
    }
  }
